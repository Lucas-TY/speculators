...
The training loss controls what the draft model is optimized for. Select it with `--loss-fn` in [`speculators train`](../cli/train.md), which accepts either a single loss name or a JSON dict for a weighted combination. It applies to Eagle-3, P-EAGLE, DFlash, DFlash2, and DSpark. MTP uses its own multi-step cross-entropy loss and ignores this flag.
...
- Use `renyi_half` with DFlash2's `--per-position-loss-weight dpard` for the detached selector-overlap D-PARD objective.
